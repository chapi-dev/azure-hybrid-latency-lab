"""Chunky workload — set-based, single round-trip per phase.

Same logical work as chatty (read N items, insert N derived rows) but:
  - 1 round-trip to read all ids in bulk
  - 1 round-trip to insert all rows via COPY

Total round-trips ~= 4 (CREATE, SELECT, COPY, INSERT run_log).
"""
    ...
    with tracer.start_as_current_span(f"BatchRun-{WORKLOAD}-{run_id}") as root:
        ...
        with psycopg.connect(conninfo, application_name=app_name) as conn:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    """
                    CREATE TABLE IF NOT EXISTS work_chunky (
                ...
                with tracer.start_as_current_span("select_bulk"):
                    cur.execute(
                        "SELECT id, sku FROM items ORDER BY id LIMIT %s;",
                        (args.items,),
                    )
                    rows = cur.fetchall()
                select_count += 1

                with tracer.start_as_current_span("copy_bulk"):
                    with cur.copy(
                        "COPY work_chunky (run_id, item_id, sku) FROM STDIN"
                    ) as cp:
                        for r in rows:
                            cp.write_row((run_id, r[0], r[1]))
                insert_count += 1
        ...
        roundtrips = select_count + insert_count + 2  # +CREATE +run_log INSERT
